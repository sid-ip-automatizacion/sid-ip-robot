...
def saveBackup_cisco_sw(ip_address, path_,clist):
    os.chdir(path_)
    device = ConnectHandler(device_type='cisco_s300', ip=ip_address, fast_cli=True,
                                username=clist[0], password=clist[1], global_delay_factor=3)
    print("backup will be stored at.. ", path_)
    output = device.send_command("show running-config",expect_string=r'#')
...
path = os.getcwd()
print(path)
credentials = ["sid-ip","sid-cw-bus1n3ss"]
saveBackup_cisco_sw('10.216.212.138',path,credentials)
'''