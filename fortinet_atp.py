...
def getVdoms(ip_address, path_,clist):
    os.chdir(path_)
    my_vdoms = []
    ...
    with open("vdoms.txt", "r") as myfile:
        for line in myfile:
            res = re.findall(r'\b: \w{3,15}\b', line)
            if len(res) > 0:
...
def saveBackup(ip_address, path_,clist):
    ...
    output = device.send_command("show", read_timeout=90)
...
def DeviceScrapping(ip_address, cust, a_path,clist):
    ...
    vdoms = getVdoms(ip_address, path_,clist)
    os.chdir(path_)
    # print(vdoms)
    device = ConnectHandler(device_type='fortinet', ip=ip_address, fast_cli=True,
                            username=clist[0], password=clist[1])
    ...
    command = 'config global'
    output = device.send_command_timing(command)
    for item in vdoms:
        command = 'sudo '+item + ' get system arp'
        print('command send : \n', command)
        output = device.send_command(command, expect_string=r'#')
        #print('command output \n: ', output)
        fileCreator('ARP_'+item+'.txt', output)
        fileCreator('ATP.txt', '********** '+command +
                    ' ********** '+'\n'+'\n'+output+'\n'+'\n'+'\n')
        IPT_ping = ip_lookup('ARP_'+item+'.txt')
        print('available ip list for '+item+' : ', IPT_ping)

        for ip_item in IPT_ping:
            if max > 0:
                try:
                    p = 'sudo '+item + ' execute ping '+ip_item
    ...
    for item in vdoms:
        try:
            command = 'sudo '+item + ' get router info routing-table all'
            output = device.send_command(command, expect_string=r'#')
            fileCreator('ATP.txt', '********** '+command +
                        ' ********** '+'\n'+'\n'+output+'\n'+'\n'+'\n')
            command = 'sudo '+item + ' execute dhcp lease-list'
            output = device.send_command(command, expect_string=r'#')
            fileCreator('ATP.txt', '********** '+command +
                        ' ********** '+'\n'+'\n'+output+'\n'+'\n'+'\n')
        except:
    ...
    body = [
        ...
        "sudo root execute ping-options repeat-count 50",
        "sudo root execute ping-options adaptive-ping enable",
        "sudo root execute ping 172.18.11.236",
        "sudo root execute ping 10.255.250.166",
        "sudo root execute ping 10.255.250.164",
        "get system status",
        "show system interface",
        "get system performance status",
        "get router info bgp summ"
    ]
